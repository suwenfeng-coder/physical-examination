...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (doctorRepository.count() == 0) {
            doctorRepository.saveAll(List.of(
                    doctor("张明", "内科", "主任医师", "13800000001"),
                    doctor("李华", "医学影像科", "副主任医师", "13800000002"),
                    doctor("王芳", "检验科", "主治医师", "13800000003")
            ));
        }
        if (itemRepository.count() == 0) {
            itemRepository.saveAll(List.of(
                    item("一般检查", "内科", "", "身高、体重、血压", "20"),
                    item("血常规", "检验科", "", "血细胞分析", "45"),
                    item("肝功能", "检验科", "", "ALT、AST等", "80"),
                    item("肾功能", "检验科", "", "肌酐、尿素氮等", "70"),
                    item("空腹血糖", "检验科", "mmol/L", "3.9-6.1", "20"),
                    item("心电图", "功能科", "", "十二导联心电图", "50"),
                    item("胸部DR", "医学影像科", "", "胸部正位片", "100"),
                    item("腹部彩超", "超声科", "", "肝胆胰脾肾", "160")
            ));
        }
        if (packageRepository.count() == 0) {
            List<CheckupItem> items = itemRepository.findAllByOrderByDepartmentAscNameAsc();
    ...
    private Doctor doctor(String name, String department, String title, String phone) {
    ...
    private CheckupItem item(String name, String department, String unit,
